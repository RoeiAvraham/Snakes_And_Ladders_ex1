...
public class GameBoard
{
    ...
    //GameBoard c'tor
    public GameBoard(Snakesandladders gameXml) throws XmlIsInvalidException
    {
        ...
        {
            m_cells[i] = new Cell(i + 1, 0, gameXml.getPlayers().getPlayer().size());
        }
    ...
    public GameBoard(final int boardSize, final int numOfSnakesAndLadders, final int numPlayers)
    {
        ...
        m_numOfSnakesAndLadders = numOfSnakesAndLadders;

        int i;
        for (i = 0; i < m_boardSize * m_boardSize; i++)
        {
            m_cells[i] = new Cell(i + 1, 0, numPlayers);
        }
        insertSnakesAndLaddersToBoard();
    }

    public void setPlayersPosFromXml(Snakesandladders gameXml, LinkedList<Player> players, LinkedList<String> playerNames) throws XmlIsInvalidException
    {
        int[] soldiersFoundCounter = new int[players.size()];
        // soldiersFoundCount keeps for each player (by index) how many of its soldiers
        // where found on the xml board.
        ...
        for (xmlPackage.Cell c : gameXml.getBoard().getCells().getCell())
        {
            for (xmlPackage.Cell.Soldiers s : c.getSoldiers())
            {
                ...
                m_cells[c.getNumber().intValue() - 1].insertSoldiers(playerIndex, s.getCount());

                for (int i = 0; i < s.getCount(); i++)
                {
                    players.get(playerIndex - 1).getSoldiersPos()[soldiersFoundCounter[playerIndex - 1]] = c.getNumber().intValue();
                    soldiersFoundCounter[playerIndex - 1]++;
                }
            }
        }
    ...
    public void NoDuplicateCellNumbersInXmlException(Snakesandladders gameXml) throws XmlIsInvalidException
    {
        List<xmlPackage.Cell> cellsList = gameXml.getBoard().getCells().getCell();
    ...
    public void insertSnakesAndLaddersFromXml(Snakes snakes, Ladders ladders) throws XmlIsInvalidException
    {
        ...
        checkSnakesAndLaddersQuantity(snakesList, ladderList);
        checkSnakesAndLaddersTopAndBottomAndFirstAndLastCells(snakesList, ladderList);
        checkNoSnakeAndLadderOnSameCell(snakesList, ladderList);
    }

    public void checkSnakesAndLaddersQuantity(List<Snake> snakesList, List<Ladder> ladderList) throws XmlIsInvalidException
    {
        if (snakesList.size() != ladderList.size())
        {
            throw new SnakesAndLaddersQuantityNotEqualXmlException();
        }
        else if (snakesList.size() > ((m_boardSize * m_boardSize) / 4))
        {
            throw new SnakesAndLaddersQuantityToBigXmlException();
        }
    }

    public void checkSnakesAndLaddersTopAndBottomAndFirstAndLastCells(List<Snake> snakesList, List<Ladder> ladderList) throws XmlIsInvalidException
    {
        for (Snake s : snakesList)
        {
            ...
            if ((s.getFrom().intValue() == 1) || (s.getFrom().intValue() == getLastCellNum()))
            {
                throw new SnakeHeadInIllegalCellXmlException();
            }
            if (s.getFrom().intValue() > getLastCellNum() || s.getTo().intValue() < 1)
            {
                throw new SnakeEndOutOfRangeXmlException();
            }
            m_cells[s.getFrom().intValue() - 1].setDest(s.getTo().intValue());
        }
        ...
        for (Ladder l : ladderList)
        {
            ...
            if ((l.getFrom().intValue() == 1) || (l.getFrom().intValue() == getLastCellNum()))
            {
            ...
            m_cells[l.getFrom().intValue() - 1].setDest(l.getTo().intValue());
        }
    }

    public void checkNoSnakeAndLadderOnSameCell(List<Snake> snakesList, List<Ladder> ladderList) throws XmlIsInvalidException
    {
        for (Snake s : snakesList)
        {
            for (Ladder l : ladderList)
            {
                if (s.getFrom().intValue() == l.getFrom().intValue()
                        || s.getFrom().intValue() == l.getTo().intValue()
                        || s.getTo().intValue() == l.getFrom().intValue()
                        || s.getTo().intValue() == l.getTo().intValue())
                {
    ...
    private void insertSnakesAndLaddersToBoard()
    {
        ...
        int maxSnakesAndLadders = m_boardSize * m_boardSize / 4;
        if (numCells % 2 == 0 && m_numOfSnakesAndLadders == maxSnakesAndLadders)
        {
